...
Transaction.init(
  {
    ...
    metadata: {
      type: DataTypes.JSONB,
      comment: "The metadata associated with the transaction (e.g., '{}').",
    },
    nonce: {
      type: DataTypes.STRING,
      comment:
        ...
        transaction_: async (_query, args, context, resolveInfo) => {
          ...
          transactions.forEach((transaction: TransactionAttributes) => {
            results.push({
              cmd: {
                meta: {
                  ...
                  creationTime: new Date(
                    parseInt(transaction.creationtime) * 1000,
                  ),
              ...
              id: transaction.id,
              result: {
                badResult: transaction.rollback
                ...
                goodResult: transaction.rollback
                  ? ""
                  : transaction.result.toString(),
                height: BigInt(0),
      ...
      resolvers: {
        Query: {
          transactionByRequestKey: async (
            ...
            const eventQueryParams = eventLimit
              ? [transaction.id, eventLimit]
              : [transaction.id];
            ...
            const transferDataPromises = transfers.map(
              async (transfer: any) => {
                let contract = null;
