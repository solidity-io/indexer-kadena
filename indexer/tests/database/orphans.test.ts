...
describe('check_backward_orphans Function with Dockerized Database', () => {
    ...
    it('should correctly identify orphan blocks', async () => {
        ...
        assert(res1.rows[0].canonical == null, 'Block 5028930 should not have been analyzed for canonical status yet, because it is in buffer zone');
        ...
        assert(nonValidatedBlock.rows[0].canonical == null, 'Block 5028931 should not have been analyzed for canonical status yet, because it is in buffer zone');
        ...
        const nonOrphanBlock = await getBlock(5028903, 1, 'mainnet01', 't-OSh9Zu0YCpwOAEsbD3pIHTEwO0c031O-hpqeSXwQc');
